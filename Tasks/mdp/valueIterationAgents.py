...
class AsynchronousValueIterationAgent(ValueIterationAgent):
    ...
    def runValueIteration(self):
        ...
        for i in range(self.iterations):
            state = i % len(self.mdp.getStates())     
            qValues = []
            for action in self.mdp.getPossibleActions(states[state]):
                qValue = self.getQValue(states[state], action)
                qValues.append(qValue)
            if len(qValues) > 0:
                self.values[states[state]] = max(qValues)
...
class PrioritizedSweepingValueIterationAgent(AsynchronousValueIterationAgent):
    ...
    def runValueIteration(self):
        # Step 1: Compute predecessors for each state
        predecessors = {}
        states = self.mdp.getStates()
        ...
        for state in states:
            if not self.mdp.isTerminal(state):
                ...
                heapq.heappush(priorityQueue, (-diff, state))
        
        # Step 4: Iteratively update values based on priority
        for _ in range(self.iterations):
            if not priorityQueue:
                break
            
            # Pop the state with the highest priority (lowest diff)
            _, state = heapq.heappop(priorityQueue)
            ...
            for predecessor in predecessors.get(state, []):
                if not self.mdp.isTerminal(predecessor):
                    ...
                    if diff > self.theta:
                        heapq.heappush(priorityQueue, (-diff, predecessor))
